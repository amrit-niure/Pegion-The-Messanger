...
{friends?.sort().map((friend) =>{
 ...
 return <li key={friend._id} className='dark:hover:text-slate-900'>
    <a href={`/dashboard/chat/${chatHerfConstructor(sessionId, friend._id)}`} className='text-gray-700 hover:text-indigo-600 hover:bg-slate-50 group flex items-center gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold justify-between '>
      <div className='flex gap-4 items-center justify-center dark:text-slate-200 dark:hover:text-slate-900 '>
...
<span>  {friend.name}</span>
 ...
 <ChevronRight className='justify-end dark:text-slate-200 dark:hover:text-slate-900 ' />
</a>
